import { formattedDate, Streak, KEY, buildStreak, updateStreak } from "./lib";
  ...
  export function streakCounter(storage: Storage, date: Date): Streak {
    ...
    if (streakInLocalStorage) {
      try {
        ...
        if (SHOULD_INCREMENT) {
            const updatedStreak = buildStreak(date, {currentCount: streak.currentCount + 1, lastLoginDate: formattedDate(date)});
